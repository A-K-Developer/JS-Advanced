...
class Garden {
  ...
  harvestPlant(plantName){
     let indexOfPlant = this.plants.findIndex(p => p.plantName == plantName);
     let plant = this.plants[indexOfPlant];
    if(indexOfPlant < 0){
      throw new Error(`There is no ${plantName} in the garden.`)
    }
    if(plant.ripe == false){
  ...
  generateReport(){
    let plantName = this.plants.map(p => p.plantName).sort((a,b) => a.localeCompare(b))
    let storageRow = '';
    ...
    let arr= [`The garden has ${this.spaceAvailable} free space left.`,`Plants in the garden: ${plantName.join(', ')}`,storageRow]
    return arr.join('\n');
